...
//		Metadata[1] = Metadata[1].replaceAll("; ",";");
		...
		if (Metadata[4].indexOf("}") < 0) year = Integer.parseInt(Metadata[4].substring(Metadata[4].indexOf("{")+1, Metadata[4].length()));
		else year = Integer.parseInt(Metadata[4].substring(Metadata[4].indexOf("{")+1, Metadata[4].indexOf("}")));	
	}
}
